        ...
        if((n = readline(sockfd , buf , MAXLINE )) == 0 ){
          ...
          if(!strncmp(buf,"/nick ",6)){
            ...
            if(!sameFlag){
              ...
              struct sockaddr_in connfdAddr; socklen_t connfdLen=sizeof(connfdAddr);
              getsockname(connfd,(SA*)& connfdAddr,&connfdLen);
              sprintf(send,"/serv Hello %s, welcome! ServerIP:%s:%d\n",clientName[i],inet_ntoa(connfdAddr.sin_addr),atoi(argv[1]) );
              mywrite(client[i],send);
